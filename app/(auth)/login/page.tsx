...
export default function LoginPage(){
  ...
  async function onSubmit(e: React.FormEvent){
    ...
    try {
      ...
      alert(e?.response?.data?.error || 'Erro ao entrar');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="mx-auto mt-20 max-w-md px-4">
      ...
      <p className="subtitle mt-1">Use seu email e senha.</p>
      ...
      <p className="mt-4 text-sm text-ink-600">Ainda não tem conta? <Link className="underline" href="/register">Criar conta</Link></p>
    </main>
  );
}
